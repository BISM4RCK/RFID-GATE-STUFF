...
const navFor = role => role === 'guard'
    ...
    : role === 'admin'
        ? [{id:'overview',icon:'⌂',label:'Overview'},{id:'logs',icon:'▤',label:'Gate Logs'},{id:'account-logs',icon:'◷',label:'Account Logs'},{id:'walkins',icon:'＋',label:'Walk-in Visitors'},{id:'blacklist',icon:'⊘',label:'Blacklist'},{id:'users',icon:'♙',label:'Users'},{id:'vehicles',icon:'▣',label:'Vehicles'},{id:'rfid',icon:'◉',label:'RFID Cards'}]
...
function StaffDashboard({user,onLogout}) {
    ...
    const content={
        ...
        users:<AdminUsers users={users}/>, vehicles:<AdminVehicles vehicles={vehicles}/>, rfid:<AdminRfid cards={cards}/>,
...
function AdminVehicles({vehicles}){return <section className="panel"><PanelHead title="Vehicles" subtitle="Registered resident vehicles."/><div className="table-wrap"><table><thead><tr><th>Plate</th><th>Resident</th><th>House</th><th>Type</th><th>Color</th><th>Status</th></tr></thead><tbody>{vehicles.map(v=><tr key={v.id}><td><strong>{v.plate_number}</strong></td><td>{v.resident_name}</td><td>{v.house_number}</td><td>{v.vehicle_type}</td><td>{v.color||'—'}</td><td>{v.status}</td></tr>)}</tbody></table></div></section>}
...
function ResidentVehicleModal({onClose,onDone}){const [f,setF]=useState({plate_number:'',vehicle_type:'car',color:''}),[error,setError]=useState(''),[busy,setBusy]=useState(false);async function submit(e){e.preventDefault();setBusy(true);try{const r=await api.post('/vehicles',f);if(r.status>=300)throw new Error(r.data?.message||'Unable to add vehicle.');onDone()}catch(e){setError(e.response?.data?.message||e.message)}finally{setBusy(false)}}return <Modal title="Add vehicle" onClose={onClose}><p className="muted">Add a vehicle to your resident account.</p>{error&&<div className="alert error">{error}</div>}<form className="form-grid" onSubmit={submit}><label>Plate number<input value={f.plate_number} onChange={e=>setF({...f,plate_number:e.target.value.toUpperCase()})} required/></label><label>Vehicle type<select value={f.vehicle_type} onChange={e=>setF({...f,vehicle_type:e.target.value})}><option>car</option><option>motorcycle</option><option>truck</option><option>other</option></select></label><label>Color<input value={f.color} onChange={e=>setF({...f,color:e.target.value})}/></label><div className="full-width button-row"><button className="secondary" type="button" onClick={onClose}>Cancel</button><button className="primary" disabled={busy}>{busy?'Saving…':'Add vehicle'}</button></div></form></Modal>}
